...
- `.planning/phases/02-search-match-intake/02-RESEARCH.md` — sections **§A.1 (Voyage wrapper skeleton lines 113–200), §A.2 (embed-text shape), §A.3 (pgvector ops + halfvec + HNSW timing), §A.4 (`match_candidates` RPC skeleton lines 287–367), §A.5 (invalidate_*_embedding trigger function lines 380–404), §B.7 (Sonnet `score_candidate_for_job` tool schema lines 462–547), §B.10 (`ai_summaries` table + RLS + FK guard lines 590–642), §B.11 (single-migration rule), §C.12 (org slug + `apply_form_enabled` column), §C.13 (Turnstile + rate-limit table), §C.14 (signed upload URL flow), §D.20 (encryption helper + `gmail_credentials` schema lines 280–331), §E.27 (FK-guard table — which tables need guards), §E.28 (type regen path)**
...
5. **Add Phase 2 deps** — `pnpm add voyageai@^0.2.0 googleapis@^144 google-auth-library@^9`. The Turnstile widget needs a React wrapper: prefer `@marsidev/react-turnstile@^1` (active maintainer, no transitive risk); if slopcheck flags it, fall back to `next-turnstile` and document the choice in the commit message. Run `npm view voyageai maintainers` and `npm view @marsidev/react-turnstile maintainers` and paste the JSON output into the commit message as a manual-verification record (Phase 1 LEARNINGS recommends this for `[ASSUMED]` packages; `02-RESEARCH.md` Package Legitimacy Audit flags both as `[ASSUMED]`).
6. **Extend `src/lib/env.ts`** — add to `server`:
   - `VOYAGE_API_KEY: z.string().min(1)`
   - `GMAIL_TOKEN_ENCRYPTION_KEY: z.string().regex(/^[0-9a-f]{64}$/, 'must be 32 random bytes hex-encoded (64 hex chars)')`
   - `GMAIL_OAUTH_CLIENT_ID: z.string().min(1).optional()` (optional in dev so the app boots before Gmail is wired in Plan 4)
   ...
   - `GMAIL_PUSH_SERVICE_ACCOUNT_EMAIL: z.string().email().optional()`
...
7. **`.env.example`** — append a `# --- Phase 2 ---` block listing every new key with one-line comments. For `GMAIL_TOKEN_ENCRYPTION_KEY`, include the comment `# Generate once with: openssl rand -hex 32` and a placeholder value of 64 zeros. For Turnstile, link to `https://dash.cloudflare.com/?to=/:account/turnstile`. For Gmail OAuth, link to `https://console.cloud.google.com/apis/credentials`.
...
- Boot the app: temporarily set `VOYAGE_API_KEY=test`, `GMAIL_TOKEN_ENCRYPTION_KEY=` to an empty string in a copy of `.env.local.test`, run `NODE_ENV=production NEXT_RUNTIME=node pnpm exec ts-node -e "require('./src/lib/env')"` (or equivalent) — expect a Zod validation error naming `GMAIL_TOKEN_ENCRYPTION_KEY`. Proves the new schema fail-closes at boot. Remove the test file after.
...
4. **`src/lib/encryption.ts`** — first line `import 'server-only'`. Tiny module (~60 lines):
   ...
   - Internal `getKey()` parses `env.GMAIL_TOKEN_ENCRYPTION_KEY` hex string → 32-byte Buffer. Cache the buffer module-scoped after first parse.
...
- create `supabase/migrations/<ts3>_gmail_credentials.sql` (table + RLS policies + set_org + set_updated_at triggers)
...
**Pattern to copy:** PATTERNS.md "Migrations" rows. RESEARCH §B.10 lines 590–642 (ai_summaries) + §B.11 (one-migration rule). RESEARCH §D.20 lines 280–331 (gmail_credentials). RESEARCH §C.13 (apply_form_rate_limits). RESEARCH §C.12 (slug format check). `supabase/migrations/20260518211005_candidate_cvs_cross_tenant_fk_guard.sql` + `20260518213836_fix_same_org_trigger_order.sql` are the canonical FK-guard pattern.
...
4. **`gmail_credentials.sql`** — paste RESEARCH §D.20 lines 280–326 verbatim. Notes:
   - `refresh_token_encrypted bytea not null` and `access_token_encrypted bytea` — but our `encryption.ts` returns packed `text` (base64-joined). **Use `text` columns, not `bytea`.** Add a comment `-- text not bytea because src/lib/encryption.ts emits base64-packed strings`. (PATTERNS.md cheat-sheet says `bytea`; RESEARCH says `bytea`; both miss that the helper emits text. Planner correction; verify in code review.)
   - Add `encryption_key_version integer not null default 1` per PATTERNS.md "Encryption helper shape" forward-compat note.
   ...
   - Triggers: `gmail_credentials_set_org` BEFORE INSERT + `gmail_credentials_set_updated_at` BEFORE UPDATE.
   - **NO `verify_same_org_check` trigger** — the only FK is to `users`, which is auth-tied; RLS `user_id = auth.uid()` is the active gate (per CONTEXT D2-20 + PATTERNS.md cheat-sheet table).
   - `unique (user_id)` — one Gmail account per recruiter (CONTEXT.md Claude's Discretion picks this; Plan 4 single-account-per-user is sufficient for anchor).
...
- Open `psql "$DB_URL"` and run each migration's documented manual smoke-test block. Specifically:
  ...
  - **`gmail_credentials` RLS:** as user-in-org-A, `select * from gmail_credentials` only returns rows where `user_id = auth.uid()`. A cross-user query (`select * from gmail_credentials where user_id = '<other-user>'`) returns zero rows.
...
- `select trigger_name, event_object_table from information_schema.triggers where event_object_table in ('ai_summaries', 'gmail_credentials') order by event_object_table, trigger_name` — confirm `ai_summaries_set_org` and `ai_summaries_verify_same_org_check` BOTH exist and sort `set_org < verify_same_org_check` (alphabetical, no bug).
- `pnpm typecheck` passes (regenerated types should include `ai_summaries`, `gmail_credentials`, `apply_form_rate_limits`, `hnsw_build_state`, `record_audit_anonymous`)
...
6. **`src/lib/db/gmail-credentials.ts`** — `import 'server-only'`. Exports (NEVER plaintext tokens — they're packed ciphertext strings):
   - `export type GmailCredentialsRow = Tables<'gmail_credentials'>`
   - `export async function getGmailCredentials(supabase, userId: string): Promise<DbResult<GmailCredentialsRow | null>>` — `maybeSingle()`; null = not connected.
   - `export async function upsertGmailCredentials(supabase, input: { userId: string; googleEmail: string; refreshTokenEncrypted: string; accessTokenEncrypted: string; accessTokenExpiresAt: string; scopes: string[]; watchExpiresAt?: string | null; lastHistoryId?: string | null }): Promise<DbResult<{ id: string }>>` — uses service-role (callers pass it in). organization_id filled by trigger.
   - `export async function updateGmailAccessToken(supabase, args: { userId: string; encryptedAccessToken: string; expiresAt: string }): Promise<DbResult<{ id: string }>>` — token refresh path.
   - `export async function updateGmailWatchState(supabase, args: { userId: string; lastHistoryId: string; watchExpiresAt: string }): Promise<DbResult<{ id: string }>>`
   - `export async function revokeGmailCredentials(supabase, userId: string): Promise<DbResult<{ id: string }>>` — set `revoked_at = now()`, null out tokens.
   - Comment block at the top: `// All token fields are CIPHERTEXT — base64-packed iv:authTag:ciphertext from src/lib/encryption.ts. The encryption boundary lives in src/lib/integrations/gmail.ts; this helper never sees plaintext.`

7. **`src/lib/supabase/middleware.ts`** — add three entries to `PUBLIC_PATHS` array (insert alphabetically near existing entries):
   - `'/api/gmail/callback'` (Plan 4 will create the route; the path-prefix match `pathname.startsWith('/api/gmail/callback/')` is the wrong shape — but the array does `pathname === p || pathname.startsWith('${p}/')` so the exact path `/api/gmail/callback` matches `===`. Confirm Plan 4 doesn't add nested sub-routes; if it does, change to `'/api/gmail'`.)
   - `'/api/gmail/push'`
   - `'/apply'`
...
- `pnpm lint` passes
- `grep -rn "import 'server-only'" src/lib/db/embeddings.ts src/lib/db/ai-summaries.ts src/lib/db/gmail-credentials.ts` — every file starts with `server-only`
- `grep -rn "createBrowserClient\|createClient.*browser" src/lib/db/embeddings.ts src/lib/db/ai-summaries.ts src/lib/db/gmail-credentials.ts` returns nothing (helpers are server-only)
- Boot `pnpm dev`; `curl -sI http://localhost:3000/apply/anything | grep -i location` returns NO `/sign-in` redirect (middleware allows the path); `curl -sI http://localhost:3000/api/gmail/push -X POST` returns 404 (route doesn't exist yet — Plan 4) rather than a sign-in redirect
