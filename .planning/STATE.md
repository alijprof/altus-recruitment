...
last_updated: 2026-06-11T19:35:16.257Z
progress:
  total_phases: 5
  completed_phases: 4
  total_plans: 31
...
**Current focus:** Phase 05 — saas shell
...
| 260605-gtj | Self-serve checkout trigger on /settings/billing — new `StartCheckoutButton` client component POSTs `{planKey}` to `/api/stripe/checkout` and redirects to hosted Stripe Checkout; the status-`none` branch now renders a 3-plan picker (Starter/Pro/Scale) with "Start 14-day trial" buttons + "Compare all plans" link. Closes Finding #1 from the 2026-06-05 prod billing smoke (endpoint worked but nothing in the UI called it). Code-review fixes applied: server-side double-subscribe guard (409 if already active/trialing/past_due), null-url guard, body-read-once in ManageBillingButton, "Unlimited seats" label, seat-divisor zero-guard. lint+typecheck green. **NOT deployed** — awaiting deploy + live pre-smoke of the new button. | 2026-06-05 | 8efa99d, d927884, 1aa02f7 | [SUMMARY](quick/260605-gtj-billing-self-serve-checkout/260605-gtj-SUMMARY.md) |
...
| 260612-0f4 | PECR one-click unsubscribe (tokenised public route, RFC 8058 one-click, durable suppression in segments + send loop) + persist per-recipient AI intro/outro (IN-02); migration pushed + types regenerated; code-reviewed (1 crit + 3 warn fixed) + verified 7/7 | 2026-06-12 | 31cfbb4, 1296658, a6c23b5, 912db08, 5686c97 | [SUMMARY](quick/260612-0f4-pecr-one-click-unsubscribe-persist-campa/260612-0f4-SUMMARY.md) |
