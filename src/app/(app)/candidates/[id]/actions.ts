...
// ---------------------------------------------------------------------------
// Signed-URL file access (D-01, Plan 07-01) MOVED, 2026-08-11.
//
// getCvFileUrlAction lived here and was awaited by cv-file-link.tsx inside a
// popup + startTransition dance. On production that promise never settled
// (see the incident note at the top of the route file), so the whole path was
// replaced by a navigable GET:
//
//     src/app/(app)/candidates/[id]/cv-file/[cvId]/route.ts
//
// which ports this logic verbatim — RLS-scoped read, isCvFileDownloadable
// mirror, PII-free Sentry capture, and the `export` audit row filed before
// delivery — and 302-redirects to the signed URL. The action had exactly one
// consumer, so it is deleted rather than left as a second, untested way in.
// ---------------------------------------------------------------------------
